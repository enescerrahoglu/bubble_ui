...
class BubbleMenuScreen extends StatefulWidget {
  const BubbleMenuScreen({super.key});

  @override
  State<BubbleMenuScreen> createState() => _BubbleMenuScreenState();
}
...
class _BubbleMenuScreenState extends State<BubbleMenuScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      screenSize = Size(query.size.width, query.size.height - query.padding.top);
      ...
      focusRect = Rect.fromLTWH(mainPadding, mainPadding, mainWidth, mainHeight);
  ...
  createBubbles() {
    ...
    for (int rowIndex = 0; rowIndex < 20; rowIndex++) {
      ...
      for (int y = 0; y < maxRowItems; y++) {
        ...
        final Offset position = Offset((y * maxBubbleWidth) + offsetWidth, (rowIndex * maxBubbleWidth));
            ...
            child: Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                    shape: BoxShape.circle, color: Colors.primaries[random.nextInt(Colors.primaries.length)]),
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Icon(
                    Icons.apple,
          ...
          child: Container(
            ...
            child: Stack(
              children: bubbles.map((item) {
                final Offset newPosition = Offset(position.dx + item.position.dx, position.dy + item.position.dy);
                final Offset centerPosition = newPosition.translate(item.radius, item.radius);
                ...
                if (focusRect.contains(centerPosition)) {
                  ...
                  if (screenRect.contains(centerPosition)) {
                    ...
                    if (centerPosition.dx < focusRect.left) {
                      xScale = 1 - ((focusRect.left - centerPosition.dx) / (focusRect.left - limitRect.left));
                      left = true;
                    } else if (centerPosition.dx > focusRect.right) {
                      xScale = 1 - ((centerPosition.dx - focusRect.right) / (limitRect.right - focusRect.right));
                      right = true;
                    }
                    // Y scale Check
                    if (centerPosition.dy < focusRect.top) {
                      yScale = 1 - ((focusRect.top - centerPosition.dy) / (focusRect.top - limitRect.top));
                      centerPosition.dy / focusRect.top;
                      top = true;
                    } else if (centerPosition.dy > focusRect.bottom) {
                      yScale = 1 - ((centerPosition.dy - focusRect.bottom) / (limitRect.bottom - focusRect.bottom));
                      bottom = true;
                    }
                    // Average Scale
                  ...
                  return Positioned(
                    ...
                    child: Transform.scale(
                      alignment: Alignment(left ? 0.5 : (right ? -0.5 : 0), top ? 0.5 : (bottom ? -0.5 : 0)),
                      scale: scale,
                      child: item.child,
                    ),
                  );
